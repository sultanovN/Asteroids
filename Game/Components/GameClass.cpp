...
void Game::GameMenu()
{
    if (GameMode == Inter::Menu && StartContinue.isClicking({ get_cursor_x(), get_cursor_y() },
        is_mouse_button_pressed(0) || is_key_pressed(VK_RETURN)))
    {
        GameMode = Inter::Game;
    }
    else if (GameMode == Inter::PauseMenu && StartContinue.isClicking({ get_cursor_x(), get_cursor_y() },
        is_mouse_button_pressed(0) || is_key_pressed(VK_RETURN)))
    {
        GameMode = Inter::Game;
    }
    else if (GameMode == Inter::Menu && Exit.isClicking({ get_cursor_x(), get_cursor_y() },
        is_mouse_button_pressed(0) || is_key_pressed(VK_RETURN)))
    {
        schedule_quit_game();
    }
